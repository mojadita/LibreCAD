...
/* canonical involute is generated by a 1.0 radius circle.
 * We consider it the next complex function:
 * (1.0 - alpha - i*phi) * exp(i*phi) 
 */
static double re_involute(const double phi, const double alpha = 0.0)
{
    return (1.0 - alpha) * cos(phi) + phi * sin(phi);
}

static double im_involute(const double phi, const double alpha = 0.0)
{
    return (1.0 - alpha) * sin(phi) - phi * cos(phi);
}

static double mod_involute(const double phi, const double alpha = 0.0)
{
    double aux = (1.0 - alpha);
    return sqrt(aux*aux + phi*phi);
}

static double arg_involute(const double phi, const double alpha = 0.0)
{
    double aux = (1.0 - alpha);
...
struct involute {
    ...
    involute(int n_t, double add, double ded, double p_ang);

    QPointF evo0(const double phi); /* involute for tooth face */
    QPointF evo1(const double phi); /* involute for tooth carving (interference) */
...
const double involute::default_eps = 8 * DBL_EPSILON;

involute::involute(int n_t, double add, double ded, double p_ang):
    n_teeth(n_t),
    addendum(add),
    dedendum(ded),
...
/* this involute calculates points for an argument phi for the
 * curve that defines de active face of the tooth.  */
QPointF involute::evo0(const double phi)
{
    double x = cos_p_angle * re_involute(phi),
           y = cos_p_angle * im_involute(phi);
 ...
 * interference. */
QPointF involute::evo1(const double phi)
{
    double x = re_involute(phi, alpha);
    double y = im_involute(phi, alpha);
 ...
 * primary involute or in the other side.  We look for a zero in this
 * function to derive the phi angle of the secondary involute at which
 * it crosses the primary.   This is the common point for both involute
 */
double involute::aux(const double phi)
{
    const double mod = mod_involute(phi, alpha);
    const double arg = arg_involute(phi, alpha);
...
/* find the common point of both involute.  this function uses two
 * values a and b of phi in the involute::evo1 curve to find a root of
 * the function involute::aux that gives the difference between the argument
 * at which the involute of the primary curve touches the base circle minus
 * the argument at which an involute that pases for the point calculated in
 * the second curve hits the base circle.  This being positive means the
 * second involute has already crossed the first.  Being negative means it
 * has not yet crossed the primary involute. */
double involute::find_common_phi_evo1(const double eps)
{
...
void lc_Geardlg::processAction(Document_Interface *doc, const QString& cmd, QPointF& center)
{
    ...
    involute ev(nteethBox->value(), /* number of teeth */
        ...
        && ev.cos2_p_angle > ev.dedendum_radius)
    {
            ...
            phi_0 = radius2arg(mod_involute(angle_2, ev.alpha) / ev.cos_p_angle);
